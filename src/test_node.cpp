/* test_node : The test program to test node class
 *
...
TEST_CASE("Orders the images correctly") {
  ...
  bool isLeft = tester.left != 0 && (tester.left->data[0].rows == test1.rows &&
                                     tester.left->data[0].cols == test1.cols);
  bool isRight =
      tester.right != 0 && (tester.right->data[0].rows == test3.rows &&
                            tester.right->data[0].cols == test3.cols);
